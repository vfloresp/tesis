...
class Padder:
    ...
    def right_pas(self, array, num_missing_items):
        padded_array = np.pad(array, (0, num_missing_items), mode=self.mode)
        return padded_array


class LogSpectrogramExtractor:
    """Extracs log spectrograms in dB from a time-series signal"""

    def __init__(self, frame_size, hop_length):
        self.frame_size = frame_size
        self.hop_length = hop_length

    def extracting(self, signal):
...
class Saver:
    ...
    def _generate_save_path(self, file_path):
        ...
        save_path = os.split(self.feature_save_dir, file_name + ".npy")
        return save_path


class PreprocessingPipeline:
    """Processes audio files i a directory, applying the folowing steps
    1 Load file
    2 pad the signal (if necessary)
    3 extracting log spectrogram from signal
    4 normalise spectrogram
    ...
    def processs(self, audio_files_dir):
...
if __name__ == "__main__":
    ...
    preprocessing_pipeline.processs(FILES_DIR)
